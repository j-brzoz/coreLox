...
static int constantInstruction(const char* name, Chunk* chunk, int32_t offset) {
    ...
    printf("'\n");
    return offset + 2; // OP_CONSTANT has two bytes, one for the opcode and one for the operand (constant's index)
}

static int32_t invokeInstruction(const char* name, Chunk* chunk, int32_t offset) {
